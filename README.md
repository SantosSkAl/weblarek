...
#### Модель продуктовый АПИ - класс ProductsAPI
- хранит реализацию АПИ, в соответствии с интерфейсом:
    ```
    ...
    ```
- содержит методы:
    - отправка и обрботка GET запроса по указанному энпоинту (uri);
    - отправка и обработка POST запроса с указанием эндпоинта (uri),
    тела запроса (data) и, опционально, смена метода запроса на 
    PUT/DELETE.
